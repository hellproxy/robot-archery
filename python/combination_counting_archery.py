...
def main():
    test_find_winner()
    test_generate_winning_sequence()
...
def print_converging_probability_distribution(n_robots, iterations):
    ...
    for robot in range(n_robots):
        p_robot = f"P({robot})"
        print(f"{p_robot:12}{padding}", end="")
    print()

    # print probabilities
    for p_distribution in islice(generate_probability_distribution(n_robots), iterations):
        for robot in range(n_robots):
            print(f"{p_distribution[robot]:.10f}{padding}", end="")
        print()
...
def generate_winning_sequence(sequence_size, sequence):
    if sequence_size == 0:
        yield sequence
    else:
        ...
        for i in range(1, sequence[0]):
            ...
            yield from generate_winning_sequence(sequence_size - 1, new_sequence)


def create_win_distribution(n_robots, final_shot):
    distribution = {robot: 0 for robot in range(n_robots)}

    sequence_size = n_robots - 2
    sequence = (final_shot, )

    for winning_sequence in generate_winning_sequence(sequence_size, sequence):
...
def test_generate_winning_sequence():
    assert set(generate_winning_sequence(1, (5, ))) == {(1, 5), (2, 5), (3, 5), (4, 5)}
    assert set(generate_winning_sequence(1, (6, ))) == {(1, 6), (2, 6), (3, 6), (4, 6), (5, 6)}
    assert set(generate_winning_sequence(2, (6, ))) == {
